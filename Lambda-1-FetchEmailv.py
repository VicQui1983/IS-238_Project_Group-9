...
def upload_and_mark_seen(mail, e_id, raw_email, bucket_name):
    """
    Uploads raw email to S3 and marks it as Seen in Gmail.
    """
    s3_key = f"{uuid.uuid4()}.eml"
    try:
        ...
        print(f"Error uploading to S3 or marking as seen: {e}")
        # Don't mark as seen if upload failed
        pass
            ...
            if status == 'OK':
                for response_part in msg_data:
                    if isinstance(response_part, tuple):
                        ...
                        upload_and_mark_seen(mail, e_id, raw_email, s3_bucket)
